...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public JsonObject handleShiroException(ShiroException e) {
        ...
        e.printStackTrace();
        return JsonObject.error("鉴权或授权过程出错：" + e.getLocalizedMessage());
    }
    ...
    @ResponseBody
    public JsonObject UnauthorizedException(UnauthorizedException e) {
        return JsonObject.error("用户没有访问权限：" + e.getLocalizedMessage());
    }

    @ExceptionHandler(IllegalArgumentException.class)
    @ResponseBody
    public JsonObject IllegalArgumentException(IllegalArgumentException e) {
        e.printStackTrace();
